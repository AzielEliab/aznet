...
const DEFAULT_REPO = "aznet";
const DEFAULT_BRANCH = "main";
const GITHUB_RELEASES = "https://github.com/AzielEliab/aznet/releases";
...
async function githubStats(env) {
  ...
  try {
    const repoRes = await fetch("https://api.github.com/repos/AzielEliab/aznet", { headers });
    if (repoRes.ok) {
    ...
    const relRes = await fetch("https://api.github.com/repos/AzielEliab/aznet/releases/latest", { headers });
    if (relRes.ok) {
...
# AZNet one-click install. Counted download via this Worker.
set -euo pipefail
HOST="${HOST}"
ASSET="${DEFAULT_ASSET}"
WORKDIR="\${AZNET_HOME:-\$HOME/aznet}"
mkdir -p "\$WORKDIR"
cd "\$WORKDIR"
echo "Downloading counted tarball from \${HOST}/download (User-Agent Mozilla/5.0)…"
curl -fsSL -A 'Mozilla/5.0' "\${HOST}/download?asset=\${ASSET}" -o "\${ASSET}"
tar -xzf "\${ASSET}"
DIR="\$(find . -maxdepth 1 -type d -name 'aznet-*' | head -n 1)"
if [ -n "\${DIR}" ]; then
  cd "\${DIR}"
fi
python3 -m venv .venv
